...
* **MATLAB App Designer:** I developed the `.mlapp` to act as the master control hub. I implemented a fixed-rate timer to parse incoming serial data every 0.1 seconds, which dynamically updates the linear/circular gauges and color-coded status lamps based on the hardware's real-time state.
